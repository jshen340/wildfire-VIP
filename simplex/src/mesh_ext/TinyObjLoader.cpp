...
template<> void Read_From_Obj_File(const std::string& file_name, Array<std::shared_ptr<TriangleMesh<3> > >& meshes)
{
    ...
    for (auto i = 0; i < shapes.size(); i++) {
        meshes[i] = std::make_shared<TriangleMesh<3>>(); tinyobj::mesh_t& mesh = shapes[i].mesh;
...
template<> void Read_From_Obj_File(const std::string& file_name, Array<std::shared_ptr<SegmentMesh<2> > >& meshes)
{
    std::cout << "2D unsupported" << std::endl;
}
