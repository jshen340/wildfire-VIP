		...
		while(ch>tol && iter_count<max_iter_num){
			...
			if(use_elementwise_bound){
				for(int i=0;i<n_var;i++){
					var_U[i]=AuxFunc::Min(var_UB[i],var[i]+movlims[i]);
					var_L[i]=AuxFunc::Max(var_LB[i],var[i]-movlims[i]);}}
			else{
				for(int i=0;i<n_var;i++){
...
public:
	...
	void MMA_Constraint(unsigned n_cons,real* cons,real* cons_grad,unsigned n_var,const real* var)
	{
		if(n_cons>0){
			...
			timer.Record("Compute Constraint");
			Compute_Constraint_Grad(var,cons_grad);
			timer.Record("Compute Constraint Gradient");
			if(write_intmed){
