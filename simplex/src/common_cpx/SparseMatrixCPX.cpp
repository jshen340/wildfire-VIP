...
template<class T>SparseMatrixCPX<T>::SparseMatrixCPX(const enum DataHolder& _side) {
	data_side = _side;
	data_type = Cuda_Real_Type<T>();
}

template<class T> SparseMatrixCPX<T>::SparseMatrixCPX(const SparseMatrixCPX<T>& copy, const DataHolder& _side)
{
	data_side = _side;
	data_type = Cuda_Real_Type<T>();
	deepcopy(copy);
}
template<class T> SparseMatrixCPX<T>::SparseMatrixCPX(const SparseMatrix<T>& copy, const DataHolder& _side)
{
	data_side = _side;
	data_type = Cuda_Real_Type<T>();
	deepcopy(copy);
...
template<class T> SparseMatrixCPX<T>& SparseMatrixCPX<T>::operator=(const SparseMatrixCPX<T>& copy)
{
	deepcopy(copy); return *this;
}

template<class T> SparseMatrixCPX<T>& SparseMatrixCPX<T>::operator=(const SparseMatrix<T>& copy)
{
	deepcopy(copy); return *this;
}

template<class T> SparseMatrixCPX<T>::operator SparseMatrix<T>&()
{
...
template<class T> void SparseMatrixCPX<T>::deepcopy(const SparseMatrixCPX<T>& copy)
{
	Clear();
...
template<class T> void SparseMatrixCPX<T>::deepcopy(const SparseMatrix<T>& copy)
{
	Clear();
...
template<class T> void SparseMatrixCPX<T>::Initialize_Color(int _color_n,int _block_size,const std::vector<int>& _color_ptr,const std::vector<int>& _color)
{
...
template<class T> void SparseMatrixCPX<T>::Clear()
{
...
template<class T> size_t SparseMatrixCPX<T>::Memory_Size()
{
	size_t size=0;
...
template<class T> void SparseMatrixCPX<T>::resize(int _m, int _n, enum DataHolder new_side)
{
...
template<class T> void SparseMatrixCPX<T>::resizeNonZeros(int _nnz)
{
	assert(data_side != UNKNOWN);
...
template<class T> cusparseSpMatDescr_t SparseMatrixCPX<T>::Get_SpMatCescr_t()
{
	if (data_side != DEVICE) {
		std::cerr << "[Error] SparseMatrixCPX::valueDescr_t(): data not on device\n";
		return nullptr;
	}
...
template class SparseMatrixCPX<float>;
template class SparseMatrixCPX<double>;
...
//
////New interfaces
//template void Residual<double>(SparseMatrixCPX<double>*, double*, double*, double*);
//template void Residual<float>(SparseMatrixCPX<float>*, float*, float*, float*);
