...
public:
	DirectSolveBlockInverse(LinearMapping *_mapping)
	{
	...
	void applyMapping(Scalar *Ap, Scalar *p) override
	{
		...
		Batch_solve_lu(
			cublasHandle, 
			CUBLAS_OP_N, //no operation on matrix
			k, //number of rows and columns
			1, //number of rhs columns
			d_A_ptr, //array of pointers with each k*k dim
			k, //leading dimension of A matrices
			d_Ipiv, //array of size k*N containing the pivoting sequence of factorization of each d_A_ptr
			d_B_ptr, //array of pointers with each k*1 dim
			k, //leading dimension of rhs
			&info, //info
			N
		);
		//cudaDeviceSynchronize();
