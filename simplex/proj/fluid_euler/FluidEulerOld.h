...
template<int d> class FluidEulerOld
{Typedef_VectorDii(d);
public:
	MacGrid<d> mac_grid;
	...
	bool use_maccormack = false;
	bool use_zero_extrapolation = false; //zero extrapolation for pos out of field during advection  
	
	FluidEulerOld(const SolverType& _solver_mode = SolverType::MULTIGRID_AUTO):projection(&mac_grid,&velocity,&type,&bc){}
