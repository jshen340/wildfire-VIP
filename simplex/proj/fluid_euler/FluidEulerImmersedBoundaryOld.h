...
template<int d> class FluidEulerImmersedBoundaryOld : public FluidEulerOld<d>
...
public:
	...
	{
		iterate_face(axis,iter,mac_grid){const VectorDi& face=iter.Coord();
			auto vf=velocity_flow(axis,face);
			auto vb=velocity_body(axis,face);
	...
	virtual void Enforce_Incompressibility()
	{
		...
		ProjectionMeta<d> projection(mac_grid,velocity,alpha,type,bc);
