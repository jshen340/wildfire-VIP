//////////////////////////////////////////////////////////////////////////
// advection-reflection fluid driver
// Copyright (c) (2021-), Fan Feng
// This file is part of SimpleX, whose distribution is governed by the LICENSE file.
//////////////////////////////////////////////////////////////////////////
#ifndef __FluidAdvectionReflectionDriver_h__
#define __FluidAdvectionReflectionDriver_h__
#include "Common.h"
#include "File.h"
#include "GeometryPrimitives.h"
#include "FluidAdvectionReflection.h"
#include "Particles.h"
#include "MarchingCubes.h"
#include "Driver.h"

template<int d> class FluidAdvectionReflectionDriver : public Driver
{
	Typedef_VectorDii(d); using Base = Driver;
public:
	FluidAdvectionReflection<d> fluid;

	FluidAdvectionReflectionDriver() :fluid(SolverType::MULTIGRID_AUTO) {}
	...
	virtual void Initialize()
	{
		...
		case 1: {	////Karman vortex street
			...
			//fluid.projection.solver_mode = SolverType::CPX_GPU;
		...
		case 2: {	////Karman vortex street with accurate solid boundary
			...
			LevelSet<d> ls_solid; ls_solid.Initialize(fluid.mac_grid.grid);
			iterate_cell(iter, fluid.mac_grid.grid) {
				const VectorDi& cell = iter.Coord();
				VectorD pos = fluid.mac_grid.grid.Center(cell);
		...
		case 4: {	////lid-driven cavity
			...
			iterate_face_in_one_dim(axis, iter, fluid.mac_grid) {
				...
				else if (face[0]<1 || face[0]>fluid.mac_grid.face_grids[axis].node_counts[0] - 2)fluid.bc.Set_Psi_N(axis, face, 0);
			}	////left and right
	...
	///////////////////////////////////////////////////////////////////////////
	////velocity setup
	void Set_Velocity_Taylor_Vortex(VectorD& vortex_p1, VectorD& vortex_p2) {
		...
		Field<real, d> sol(fluid.mac_grid.grid.cell_counts, (real)0);
		FaceField<real, d> alpha(fluid.mac_grid.grid.cell_counts, (real)1);
		PoissonOld<d, real> poisson(fluid.mac_grid, alpha, wz, sol, fluid.bc);
		...
		iterate_face(axis, iter, fluid.mac_grid) {
			...
			if (axis == 0) { fluid.velocity(1, face) = (sol(cell_right) - sol(cell_left)) / fluid.mac_grid.grid.dx; }
			else if (axis == 1) { fluid.velocity(0, face) = (sol(cell_left) - sol(cell_right)) / fluid.mac_grid.grid.dx; }
		}
	}
};
#endif
